...
object AstroCalculator {
    private val signs = arrayOf("حمل","ثور","جوزا","سرطان","اسد","سنبله","میزان","عقرب","قوس","جدی","دلو","حوت")
    ...
    private fun kepler(m0:Double,e:Double):Double { var E=toRad(norm(m0)); repeat(12){ E -= (E-e*sin(E)-toRad(norm(m0)))/(1-e*cos(E)) }; return E }
    ...
    private fun moonLon(d:Double, sunLon:Double):Double {
        val N=toRad(norm(125.1228-.0529538083*d)); val i=toRad(5.1454); val w=toRad(norm(318.0634+.1643573223*d)); val a=60.2666; val e=.0549; val M=norm(115.3654+13.0649929509*d); val E=kepler(M,e); val xv=a*(cos(E)-e); val yv=a*sqrt(1-e*e)*sin(E); val v=atan2(yv,xv); val r=hypot(xv,yv); val xh=r*(cos(N)*cos(v+w)-sin(N)*sin(v+w)*cos(i)); val yh=r*(sin(N)*cos(v+w)+cos(N)*sin(v+w)*cos(i)); val lon=norm(toDeg(atan2(yh,xh)))
        // Main lunar evection/variation corrections, adequate for sign-level research UI.
    ...
    private fun wrapRad(x:Double):Double { var v=x%(2*Math.PI); if(v<0)v+=2*Math.PI; return v }
    ...
    fun calculate(local:LocalDateTime, tzOffset:Double, lat:Double, lon:Double, equalHouses:Boolean):Chart {
        ...
        val names=listOf("Mercury","Venus","Mars","Jupiter","Saturn","Uranus","Neptune"); val lons=mutableMapOf("خورشید" to sun,"ماه" to moon)
        ...
        val (asc,mc)=ascendant(jd,lat,lon); val ascSign=floor(asc/30).toInt(); val bodies=lons.map{(n,l)-> val s=floor(l/30).toInt(); val h=if(equalHouses) floor(norm(l-asc)/30).toInt()+1 else ((s-ascSign+12)%12)+1; Body(n,l,s,l%30,h)}
        val aspects=mutableListOf<String>(); val list=bodies
        for(i in list.indices) for(j in i+1 until list.size){ val diff=abs(norm(list[i].lon-list[j].lon).let{if(it>180)360-it else it}); val targets=listOf(0.0,60.0,90.0,120.0,180.0); val labels=listOf("هم‌نشینی","تسدیس","تربیع","تثلیث","مقابله"); for(k in targets.indices){ val orb=if(k==0||k==4)8.0 else 6.0; if(abs(diff-targets[k])<=orb){aspects.add("${list[i].name} ${labels[k]} ${list[j].name} (${diff.roundToInt()}°)");break} } }
